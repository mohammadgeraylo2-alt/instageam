...
def snapshot_usage():
    """مصرف لحظه‌ای همه‌ی کاربرا رو تو usage_totals.json ذخیره می‌کنه.
    همیشه باید درست قبل از هر ری‌استارت Xray صدا زده بشه، چون ری‌استارت
    شمارنده‌ی مصرف داخلی Xray رو صفر می‌کنه و بدون این snapshot، اون بخش
    از مصرف واقعی برای همیشه گم می‌شه (یا برعکس، حساب اشتباه می‌شه)."""
    clients = load_json(CLIENTS_PATH, [])
    if not clients:
        return {}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _handle_create(self):
        ...
        try:
            ...
            self._send(400, {"error": "gb must be a positive number, days a positive integer"})
            return
